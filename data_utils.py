...
def istopk(data, nums, rho=1.0):
    ...
    """
    mask = torch.zeros(len(data), dtype=torch.bool) #mask長度和data長度一樣，並且爲bool類型，默認爲0
    row_indices_per_batch = data._batchwise_row_indices#对每行进行索引
    # print(f'nums -> {nums}')
    for row_indices, N in zip(row_indices_per_batch, nums):#相当于遍历每个batch，和他对应的nums
        k = int(min(len(row_indices), N*rho))#取最小值，一般是N*rho
        _, indices = torch.topk(data.F[row_indices].squeeze().detach().cpu(), k)# must CPU.#返回值最大的k个元素
        #print(data.F)
